...
async def main():
    ...
    def spawn_enemy(enemy_spawn):
        enemy_spawn += 1
        enemies.add(Enemy(1000,64,64))
        return enemy_spawn

    enemy_spawn = spawn_enemy(enemy_spawn)
    ...
    def update(punch_r,fireball_r,arrow_r):
        screen.fill(black)
    ...
    def scroll_bg(amount, scroll, total_offset, enemy_spawn):
        ...
        if not player.sprite.jumping and not player.sprite.fall:
            ...
            jump(15)
        update(punch_r,fireball_r,arrow_r)
    ...
    def gen_plat(scroll,plat_level,plat_num):
    ...
    for plat_num in range(10):
        gen_plat(scroll,plat_level,plat_num)
    ...
    while running:
        ...
        if keys[pygame.K_LEFT]:
            scroll_bg(-6, scroll, total_offset, enemy_spawn)
